...
class Display:
    def __init__(self, width=1920, height=1080, image=None, name="noname", output=None, no_mouse=False):
        self.width=width
        self.height=height
        ...
        if output is not None:
            ...
            self.writer = cv2.VideoWriter(output, fourcc, 5.0, (width, height))
...
def faf_display(list_of_stuff, title="no title", width=1280, height=720):
    img=None
    objs=[]
    ...
    display=Display(width=width, height=height, no_mouse=True)
    display.clear()
