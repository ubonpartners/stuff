"""Cairo-based ARGB drawing (boxes, lines, circles, text, images) with normalized [0,1] coords and NumPy/OpenCV interop."""
import time
...
class ARGBdraw:
    ...
    def img(self, img, box):
        ...
        # Convert BGR or BGRA to Cairo ARGB
        if resized.shape[2] == 3:
            # Add alpha channel
            resized = cv2.cvtColor(resized, cv2.COLOR_BGR2BGRA)

        # BGRA -> ARGB for Cairo (swap B and R)
        argb = resized[:, :, [2, 1, 0, 3]]

        # Convert to 1D byte buffer
        data = argb.flatten().tobytes()

        # Create an ImageSurface from the image data
        img_surface = self.cairo.ImageSurface.create_for_data(
            bytearray(data), self.cairo.FORMAT_ARGB32, pw, ph, pw * 4
        )

        # Paint it onto the main surface
        ctx=self.ctx
        ctx.set_source_surface(img_surface, px, py)
        ctx.paint()
    ...
    def get_scaled_numpy_view(self, width, height):
        ...
        # Extract data from scaled surface (dimensions are width x height from scaled_surface)
        buf = scaled_surface.get_data()
        bgra = np.ndarray((height, width, 4), dtype=np.uint8, buffer=buf)
        argb = bgra[..., [3, 2, 1, 0]]
        return argb
